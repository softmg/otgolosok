...
Status: in progress since 2026-09-25, фазы 1–3 и 5 выполнены: выложено, повторный прогон 62 мест — 35 готовы (`docs/agents/text-generation-verification.md`); фаза 4 и массовый перезапуск ждут решения пользователя.
...
2. Повтор через `retryBatchItem` с `restartFrom:"auto"` (поиск не повторяется):
   ...
   **Расхождение:** для `REVIEW_REQUIRED` `auto` бесполезен (evidence старого формата), добавлен
   `restartFrom: "facts"` в `retryBatchItem` (коммит f2e3f38). Массового инструмента нет: для 62 мест использовать HTTP-эндпоинт повтора из админки
